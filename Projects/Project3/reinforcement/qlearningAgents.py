    ...
    def computeValueFromQValues(self, state):
        ...
        if len(actions)>0:
          ...
          for action in actions:
            tmp_Q[action] = self.getQValue(state, action)
          return tmp_Q[tmp_Q.argMax()]    
        else:
          return 0.0
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features:
          q += features[feature] *self.getWeights()[feature] #calculate dot product
        return q
    ...
    def update(self, state, action, nextState, reward):
        ...
        weights, features = self.getWeights(), self.featExtractor.getFeatures(state, action)
        for f in features:
          weights[f] += self.alpha*delta*features[f]
